...
scatter(test_df.Rab, test_df.Rbb, xlabel="predator to prey beta diversity", ylabel="prey to predator beta diversity", group = test_df.species, markershape = [:circle :rect :star5 :diamond :star4 :cross :xcross :utriangle], markersize= 6, palette = :Dark2_8, markerstrokewidth=0, size=(1000,600), left_margin=10mm, right_margin=10mm, top_margin=10mm, bottom_margin=10mm, foreground_color_legend=nothing, background_color_legend=:seashell)
...
for i in 1:28
    ...
    xaxis!("Longitude")
    yaxis!("Latitude")
    scatter!(occ_gbif_iucn[occ_gbif_iucn.species.==names(names_df)[i],:latitude], occ_gbif_iucn[occ_gbif_iucn.species.==names(names_df)[i],:longitude], markerstrokewidth=0, markeralpha=0.5, markersize=2, title=names(names_df)[i], legend=:none)
